...
import sqldb from '../config/sqldb.js';
  ...
  export const addToCart = (req, res) => {
    ...
    // 1. Check if variation exists and has sufficient stock
    sqldb.query(
        ...
        (err, variationResults) => {
            ...
            sqldb.query(
                `SELECT cart_item_id, quantity FROM cart_items 
                 WHERE customerID = ? AND ProductID = ? AND VariationID = ?`,
                [userId, productId, variationId],
                (err, existingItems) => {
                    if (err) {
                        console.error('DB Error (cart lookup):', err);
                        return res.status(500).json({ success: false, message: 'Database error', error: err.message });
                    }

                    if (existingItems.length > 0) {
                        // 3. If exists, update quantity
                        const existingQty = existingItems[0].quantity;
                        ...
                        sqldb.query(
                            ...
                            (err, updateResult) => {
                                if (err) {
                                    console.error('DB Error (update quantity):', err);
                                    return res.status(500).json({ success: false, message: 'Database error', error: err.message });
                                }
                        ...
                        sqldb.query(
                            `INSERT INTO cart_items (customerID, ProductID, VariationID, quantity)
                             VALUES (?, ?, ?, ?)`,
